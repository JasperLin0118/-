...
void add_mat(const struct mat *m1_p, const struct mat *m2_p, struct mat *result_p) { // matrix addition
    ...
    int _i = m1_p->row, _j = m1_p->col;
    result_p->row = _i, result_p->col = _j;
    for(int i = 0 ; i < _i ; i++){
        for(int j = 0 ; j < _j ; j++){
...
void mul_mat(const struct mat *m1_p, const struct mat *m2_p, struct mat *result_p) { // matrix multiplication
    ...
    for(int i = 0 ; i < _i ; i++){
        for(int j = 0 ; j < _j ; j++){
            for(int k = 0 ; k < n ; k++){
                result_p->value[i][j] = result_p->value[i][j] + m1_p->value[i][k]*m2_p->value[k][j];
            }
        }
    }
}
